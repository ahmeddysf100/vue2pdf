<template>
  <CustomModal :modelValue="showModal" @update:modelValue="colseModal">
    <template #header>
      <h3>ID:{{ person.id }}</h3>
    </template>

    <template #body>
      <div ref="contentRef" class="card-client">
    ...
    <template #footer>
      <div class="container">
        <!-- <router-link :to="{ name: 'PdfReview' }" @click="showModal = false">
        Go to PDF Review
      </router-link> -->
        <button class="btn btn-save" @click="generatePDF">Convert</button>
      </div>
    </template>
  </CustomModal>
</template>
...
const emit = defineEmits(["update:showCard"]);
const person = ref(props.person);

const colseModal = (val) => {
  showModal.value = val;
  emit("update:showCard", val);
  console.log("card2pdf", showModal.value);
};
